...
import ShopCard from '../shared/components/cards/shop-cart';
...
const Page = () => {
  const {
    ...
    queryKey: ["products"],
    queryFn: async () => {
      const res = await axiosInstance.get("/recommendation/api/get-recommendation-products?page=1&limit=10");
        ...
        {!offersLoading && !isError && (
          <>
            {offers && offers.length > 0 ? (
              ...
              <p className="text-center text-gray-500 py-6">No offers available</p>
            )}
          </>
        )}
      </div>
