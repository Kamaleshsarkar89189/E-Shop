                ...
                <div className="w-full flex flex-col md:flex-row">
                    ...
                    <div className='w-full md:w-1/2 md:pl-8 mt-6 md:mt-0'>
                        ...
                        <div className='border-b relative pb-3 border-gray-200 flex items-center justify-between'>
                            ...
                            <button
                                className="flex cursor-pointer items-center justify-center gap-2 px-4 py-3 bg-blue-600 text-white rounded-lg shadow-md hover:bg-blue-700 transition-colors"
                                onClick={() => handleChat()}
                            >
                                💬 Chat with Seller
                            </button>
                        ...
                        <p className='mt-2 text-gray-700 whitespace-pre-wrap w-full'>
                            {data?.short_description ||
                                `Lorem, ipsum dolor sit amet consectetur adipisicing elit. Perspiciatis, quisquam impedit facere odit adipisci eligendi voluptatibus possimus temporibus, libero vitae, sint ab. Numquam reprehenderit iste a commodi earum, consequuntur, id, suscipit esse nisi aliquam distinctio voluptatum eaque non iure voluptas tenetur? Molestias id quibusdam enim doloribus praesentium ex veritatis exercitationem?`}
                        </p>
                                ...
                                <div>
                                    ...
                                    <div className="flex gap-2 mt-1">
                                        {data.colors.map((color: string, index: number) => (
                                            <button
                                                key={index}
                                                className={`w-8 h-8 cursor-pointer rounded-full border-2 ${isSelected === color
                                ...
                                <div>
                                    ...
                                    <div className="flex gap-2 mt-1">
                                        {data.sizes.map((size: string, index: number) => (
                                            <button
                                                key={index}
                                                className={`px-4 py-1 cursor-pointer rounded-md transition-colors ${isSizeSelected === size
                        ...
                        <div className="mt-5 flex items-center gap-5">
                            <strong className="text-sm font-medium">Quantity:</strong>
                            <div className="flex items-center rounded-md">
                                <button
                                    className="px-3 cursor-pointer py-1 bg-gray-300 hover:bg-gray-400 rounded-l-md transition-colors"
                                    onClick={() => setQuantity((prev) => Math.max(1, prev - 1))}
                                >
                                    -
                                </button>
                                <span className="px-4 bg-gray-100 py-1 text-center font-semibold">
                                    {quantity}
                                </span>
                                <button
                                    className="px-3 py-1 cursor-pointer bg-gray-300 hover:bg-gray-400 rounded-r-md transition-colors"
