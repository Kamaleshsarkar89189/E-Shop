...
export const getAllEvents = async (
    ...
    try {
        ...
        // If there is an events then only active this function
        ...
        // OrderBy starting date instead of totalSales
        const [events, total, top10BySales] = await Promise.all([
            prisma.products.findMany({
                skip,
                where: baseFilter,
...
// Top shops fetch without cheking highest order
export const topShops = async (
    req: Request,
    res: Response,
    next: NextFunction
    ...
    try {
        ...
        // Add totalSales (sum of order totals) to each shop
        const enrichedShops = shops.map((shop) => ({
            ...shop,
            totalSales: shop.orders.reduce((sum, order) => sum + order.total, 0),
        }));

        // Sort by sales, take top 10
        const top10Shops = enrichedShops
            .sort((a, b) => b.totalSales - a.totalSales)
            .slice(0, 10);
