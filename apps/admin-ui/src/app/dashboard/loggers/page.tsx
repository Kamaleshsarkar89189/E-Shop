    ...
    <div className="w-full min-h-screen p-8 bg-black text-white text-sm">
      ...
      <div className="flex justify-between items-center mb-3">
        ...
        <button
          ...
          className="text-xs px-3 flex items-center justify-center gap-1 py-2 bg-gray-800 hover:bg-gray-700 rounded transition-colors"
        >
      ...
      <div
        ref={logContainerRef}
        className="bg-black font-mono border border-gray-800 rounded-md p-4 h-[600px] overflow-y-auto shadow-2xl"
      >
        {filteredLogs.length === 0 ? (
          <p className="text-gray-500 animate-pulse">Waiting for logs ...</p>
        ) : (
          filteredLogs.map((log, idx) => (
            <div key={idx} className="whitespace-pre-wrap mb-1 hover:bg-white/5 p-0.5 rounded">
              <span className="text-gray-500">
                [{new Date(log.timestamp).toLocaleTimeString()}]
              </span>{" "}
              <span className="text-purple-400 font-semibold">{log.source}</span>{" "}
              <span className={`font-bold ${typeColorMap[log.type]}`}>
